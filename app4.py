...
def get_gspread_client():
    # 기존에 Secrets에 저장해둔 구글 키 정보를 그대로 활용하되, 주소창만 분리합니다.
...
if menu == "📝 학생용 설문조사 입력":
    ...
    with st.form("survey_form"):
        ...
        q8_ans = st.multiselect("8. 쓰레기를 처리하는 방법 중 가장 적절한 2가지를 고르면?", ["1", "2", "3", "4"], format_func=lambda x: {"1": "① 전국의 쓰레기를 한곳에 모아 한꺼번에 처리한다.", "2": "② 다시 사용 가능한 유리병은 되돌려주고 돈을 받는다.", "3": "③ 집에서 태울 수 있는 쓰레기는 각자가 태워서 처리한다.", "4": "④ 페트병을 버릴 때는 병에 부착된 비닐และ 페트병을 따로 분리하여 버린다."}[x])
        ...
        practice_questions = {
            19: "나는 물을 사용하지 않을 경우, シャワー기나 수도꼭지를 잠글 것이다.",
        ...
        if submitted:
            try:
                # 🔄 버튼 누른 즉시 최신 행 상태 로드 (동시접속 유실 차단)
                ...
                if not df_latest.empty and "학생 식별자" in df_latest.columns:
                    ...
                    if not match.empty:
                        # 완벽한 동시 제어와 롤백 방지를 위해 구글 API 행 삭제 후 즉시 신규 어펜드 처리
